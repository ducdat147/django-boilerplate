...
def cohort_random_data():
    ...
    for index, group in enumerate(groups):
        ...
        headers.append(
            {
                "title": _("Group #{}").format(group),
                "subtitle": _("Total {}").format(intcomma(total)),
            }
        )
...
def random_data(request):
    ...
    return {
        ...
        "kpi": [
            {
                "title": _("Product A Performance"),
                "metric": f"${intcomma(f'{random.uniform(1000, 9999):.02f}')}",
                "footer": mark_safe(
                    '<strong class="text-green-700 font-semibold dark:text-green-400">+{}%</strong>&nbsp;progress from last week'.format(
        ...
        "progress": [
            {
                "title": _("Social marketing e-book"),
                "description": f"${intcomma(f'{random.uniform(1000, 9999):.02f}')}",
                "value": random.randint(10, 90),
            },
            {
                "title": _("Freelancing tasks"),
                "description": f"${intcomma(f'{random.uniform(1000, 9999):.02f}')}",
                "value": random.randint(10, 90),
            },
            {
                "title": _("Development coaching"),
                "description": f"${intcomma(f'{random.uniform(1000, 9999):.02f}')}",
                "value": random.randint(10, 90),
            },
            {
                "title": _("Product consulting"),
                "description": f"${intcomma(f'{random.uniform(1000, 9999):.02f}')}",
                "value": random.randint(10, 90),
            },
            {
                "title": _("Other income"),
                "description": f"${intcomma(f'{random.uniform(1000, 9999):.02f}')}",
                "value": random.randint(10, 90),
            },
            {
                "title": _("Course sales"),
                "description": f"${intcomma(f'{random.uniform(1000, 9999):.02f}')}",
                "value": random.randint(10, 90),
            },
            {
            ...
            {
                "title": _("Customer Retention Rate"),
                "description": f"${intcomma(f'{random.uniform(1000, 9999):.02f}')}",
                "value": random.randint(10, 90),
            },
        ],
        ...
        "performance": [
            {
                ...
                "metric": "$1,234.56",
                "footer": mark_safe(
                    _('<strong class="text-green-600 font-medium">+3.14%</strong>&nbsp;progress from last week')
                ),
                "chart": {
                    "labels": [WEEKDAYS[day % 7] for day in range(1, 28)],
                    "datasets": [
